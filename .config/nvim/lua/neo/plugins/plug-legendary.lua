return require('neo/lib/functions/imports')('legendary', function(plugin)
  local opts = {
    noremap = true,
    silent = true
  }
  ...
  local finder
  finder = function(command)
    return cmd("lua require('neo/etc/commands/finder').find_${command}()")
  end
  local t
  t = function(command)
    return cmd("TroubleToggle " .. tostring(command))
  end
  local l
  l = function(command)
    return ":Legendary " .. tostring(command)
  end
  local b
  b = function(command)
  ...
  local ec
  ec = function(command)
    return co(tostring(e(command)))
  end
  local eb
  eb = function(command)
    return bs(tostring(e(command)))
  end
  return plugin.setup({
