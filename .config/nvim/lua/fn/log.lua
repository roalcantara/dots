  ...
  if
    not pcall(function()
      local logger_ok, logger = pcall(function()
        return require('structlog').get_logger 'neo'
      end)
...
function Log:init()
  ...
  local log_level = Log.levels[(neo.log.level):upper() or 'WARN']
  structlog.configure {
    neo = {
      pipelines = {
        {
  ...
  local logger = structlog.get_logger 'neo'

  -- Overwrite `vim.notify` to use the logger
  if neo.log.override_notify then
...
function Log:get_path()
  return string.format('%s/%s.log', get_cache_dir(), 'neo')
end

---Add a log entry at TRACE level
---@param msg any
---@param event any
function Log.t(msg, event)
