...
" Helper to get autload functions easily - used with my `auto` snippet
if !exists('*util#autoload')
  function! util#autoload() abort
...
if !exists('*util#refs')
  function! util#refs(...) abort
    ...
    call setloclist(0, l:loc_objects)
    lwindow
  ...
  " Find project root
if !exists('*util#project_root')
  function! util#project_root(...) abort
...
" Convert dictioary to formatted json
if !exists('*util#dict_to_json')
  function! util#dict_to_json(dict) abort
    ...
    call nvim_buf_set_lines(l:buffer_number, 0, -1, 1, std#json#format(util#json_encode(a:dict)))
    silent! call dictwatcherdel(a:dict, '*', 's:dict_watcher_func')
    function! s:dict_watcher_func(d, k, z) abort closure
      call nvim_buf_set_lines(l:buffer_number, 1, -1, 1, std#json#format(util#json_encode(a:d)))
    endfunction
    call dictwatcheradd(a:dict, '*', function('s:dict_watcher_func'))
  endfunction
endif

" How long <command> takes to execute <n> times (in seconds).
if !exists('*util#prof')
  function! util#prof( command, n )
...
" visual code review
if !exists('*util#putsv')
  function! util#putsv() abort
...
if !exists('*util#reload')
  function! util#reload() abort
    if &filetype == 'vim'
      :silent! write
      :source %
    elseif &filetype == 'lua'
