...
const extract_kb_paths = () => {
  const yamlSourcesPath = expandHome(process.argv[3]?.trim() || `~/.config/kodexb/sources`)
  puts(`(KB/PATHS) yamlSourcesPath: '${yamlSourcesPath}' ✔︎`)
  const dbPath = expandHome(process.argv[4]?.trim() || `~/.config/kodexb/knowledge.sqlite`)
  puts(`(KB/PATHS) dbPath: '${dbPath}' ✔︎`)
  const assetsPath = expandHome(process.argv[5]?.trim() || `~/.config/kodexb/assets`)
